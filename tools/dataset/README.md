...
Recommended split for screen data (safe, no leakage):

- Originals only: `datasets/real_scan/board_binary_domain/screenshot/{board,no_board}`
- Originals only: `datasets/real_scan/board_binary_domain/photo_screen/{board,no_board}`
- Any `*_aug` folders are TRAIN-OFF in this script version and must not be passed.

Training command (original folders only):
...
Calibration command (original folders only):
...
Both scripts now block `*_aug/*augmented*` roots to prevent train/val/test leakage.
