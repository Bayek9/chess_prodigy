  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: ((ctx, constraints) {
        final raw = min(constraints.maxWidth, constraints.maxHeight);
        final size = ((raw ~/ 8) * 8).toDouble();
        var boardSizeProportion = (showCoordinatesZone ? 0.9 : 1.0);
        final boardRaw = size * boardSizeProportion;
        final boardSize = ((boardRaw ~/ 8) * 8).toDouble();
        return Stack(
...
class _ChessboardState extends State<_Chessboard> {
  ...
  void _handlePanUpdate(DragUpdateDetails details) {
    ...
    setState(() {
      _dndDetails!.position = (p.dx, p.dy - cellSize * _ChessBoardPainter._dragLift);
      _dndDetails!.endCell = (file, rank);
    });
  }
        ...
        if (isPieceOfPlayerInTurn) {
          ...
          if (currentlySelected) {
            ...
            setState(() {
              _possibleMoves = moves;
              _tapStart = (file, rank);
              _pressedFromSquare = cellCoordinate;
            });
          }
        } else {
        ...
        children: [
          CustomPaint(
            painter: _ChessBoardPainter(
              ...
              pressedFromSquare: _pressedFromSquare,
              arrow: widget.arrow,
              ...
              // On dessine nous-memes les indicateurs dans le painter
              hasCustomNormalIndicator: false,
              hasCustomCaptureIndicator: false,
            ),
...
class _ChessBoardPainter extends CustomPainter {
  ...
  final String? pressedFromSquare;
  final BoardArrow? arrow;
  final Map<String, Color> cellHighlights;
  final bool highlightLastMoveSquares;
  ...
  _ChessBoardPainter({
    ...
    required this.pressedFromSquare,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 2) halo au-dessus des points (seulement en drag)
    _drawSelectionHalo(canvas, size);

    // 3) pièces fixes
    _drawPieces(canvas, size);
    _drawLastMoveArrow(canvas, size);

    // 4) pièce en drag au-dessus de tout
  ...
  void _drawCells(Canvas canvas, Size size) {
    ...
    for (final row in [0, 1, 2, 3, 4, 5, 6, 7]) {
      for (final col in [0, 1, 2, 3, 4, 5, 6, 7]) {
        ...
        final squareName = "${String.fromCharCode('a'.codeUnitAt(0) + file)}"
            "${String.fromCharCode('1'.codeUnitAt(0) + rank)}";
        ...
        final highlightColor = cellHighlights[squareName];
        final lastFrom = highlightLastMoveSquares ? arrow?.from : null;
        final lastTo = highlightLastMoveSquares ? arrow?.to : null;
        final isPressedStart = pressedFromSquare == squareName;
        ...
        final top = (row * cellSize).roundToDouble();
        final right = ((col + 1) * cellSize).roundToDouble();
        final bottom = ((row + 1) * cellSize).roundToDouble();
  ...
  void _drawSelectionHalo(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, cellSize * _haloRadiusFactor, p);
  }
  ...
  void _drawPossibleMoves(Canvas canvas, Size size) {
    ...
    for (final moveSquare in possibleMoves) {
      ...
      final isWhiteCell = ((col + row) % 2 == 0);
      final indicatorColor = isWhiteCell ? _indicatorLight : _indicatorDark;
      final dotRadius = cellSize * 0.17;
