...
class Opencv4Plugin: FlutterPlugin, MethodCallHandler {
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    when (call.method) {
      ...
      "refineBoardCorners" -> {
        try {
          RefineBoardCornersFactory.process(
            call.argument<Int>("pathType") as Int,
            call.argument<String>("pathString") as String,
            call.argument<ByteArray>("data") as ByteArray,
            call.argument<ArrayList<Double>>("corners") as ArrayList<Double>,
            call.argument<Double>("roiPaddingRatio") as Double,
            call.argument<Double>("minAreaRatio") as Double,
            call.argument<Double>("approxEpsilonRatio") as Double,
            call.argument<Int>("cannyLow") as Int,
            call.argument<Int>("cannyHigh") as Int,
            call.argument<Int>("subPixWinSize") as Int,
            call.argument<Int>("maxCandidates") as Int,
            result
          )
        } catch (e: Exception) {
          result.error("OpenCV-Error", "Android: " + e.message, e)
